...
public class EbonyStarItemClass extends RangedWeaponItem implements Vanishable {
    ...
    public static final Predicate<ItemStack> EBONYSTAR_PROJECTILES = (stack) -> stack.isOf(ModItems.SCULK_PILE);

    public EbonyStarItemClass(Item.Settings settings) {
        super(settings);
    }
    ...
    private static boolean loadProjectiles(LivingEntity shooter, ItemStack crossbow) {
        ...
        for(int k = 0; k < j; ++k) {
            ...
            if (itemStack.isEmpty() && bl) {
                itemStack = new ItemStack(ModItems.SCULK_PILE);
    ...
    private static boolean loadProjectile(LivingEntity shooter, ItemStack crossbow, ItemStack projectile, boolean simulated, boolean creative) {
        if (projectile.isEmpty()) {
            ...
            boolean bl = creative && projectile.getItem() instanceof EbonyStarAmmoClass;
            ItemStack itemStack;
    ...
    private static void shoot(World world, LivingEntity user, Hand hand, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean creative, float speed, float divergence, float simulated) {
        if (!world.isClient) {
            ProjectileEntity projectileEntity;
                projectileEntity =  new EbonyStarProjectileEntity(user, world);
            if (user instanceof CrossbowUser) {
                CrossbowUser crossbowUser = (CrossbowUser)user;
            ...
            world.playSound((PlayerEntity)null, user.getX(), user.getY(), user.getZ(), SoundEvents.ENTITY_WARDEN_TENDRIL_CLICKS, SoundCategory.PLAYERS, 1.0F, soundPitch);
        }
    }

    private static PersistentProjectileEntity createArrow(World world, LivingEntity entity, ItemStack crossbow, ItemStack arrow) {
        ArrowItem arrowItem = (ArrowItem)(arrow.getItem() instanceof ArrowItem ? arrow.getItem() : ModItems.SCULK_PILE);
